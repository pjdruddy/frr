...
/* REMOVE PJDR */
void zebra_vxlan_mac_get_access_info(zebra_mac_t *mac, struct interface **ifpP,
				     vlanid_t *vid);
...
static void zevi_print_mac_hash_all_vni(struct hash_bucket *bucket, void *ctxt)
{
	...
	if (wctx->print_dup)
		...
		hash_iterate(zevi->mac_table, zebra_evpn_print_mac_hash, wctx);
	wctx->json = json;
	if (json) {
		if (wctx->count)
					       ...
					       void *ctxt)
{
	...
	if (wctx->print_dup)
		...
		hash_iterate(zevi->mac_table, zebra_evpn_print_mac_hash_detail,
			     wctx);
	wctx->json = json;
	if (json) {
		if (wctx->count)
				     ...
				     uint32_t n_flags)
{
	...
	if (!zmac)
		zmac = zebra_evpn_mac_lookup(zevi, mac);
				   ...
				   bool dp_static)
{
	...
	zmac = zebra_evpn_mac_lookup(zevi, macaddr);
	if (!zmac) {
		...
		zmac = zebra_evpn_mac_add(zevi, macaddr);
		if (!zmac) {
					...
					vni_t vni, struct ipaddr *ip)
{
	...
	mac = zebra_evpn_mac_lookup(zevi, &nbr->emac);
					...
					struct ipaddr *ip)
{
	...
	zmac = zebra_evpn_mac_lookup(zevi, &n->emac);
	if (!zmac) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
			      ...
			      struct ethaddr *macaddr, vlanid_t vid)
{
	...
	return zebra_evpn_del_local_mac(zevi, macaddr, ifp);
}
				     ...
				     bool dp_static)
{
	...
	return zebra_evpn_add_update_local_mac(zvrf, zevi, ifp, macaddr, vid,
						 ...
						 void *arg)
{
	...
	if (CHECK_FLAG(zn->flags, ZEBRA_NEIGH_LOCAL) &&
		...
		zmac = zebra_evpn_mac_lookup(wctx->zevi, &zn->emac);
		if (!zmac)
			return;
