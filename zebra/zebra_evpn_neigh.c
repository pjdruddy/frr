...
/**************************** SYNC neigh handling **************************/
static bool zebra_evpn_neigh_is_static(zebra_neigh_t *neigh)
{
	return !!(neigh->flags & ZEBRA_NEIGH_ALL_PEER_FLAGS);
}

static bool zebra_evpn_neigh_is_ready_for_bgp(zebra_neigh_t *n)
{
	bool mac_ready;
	bool neigh_ready;
				  ...
				  uint32_t neigh_flags, uint32_t seq)
{
	...
	return zebra_evpn_macip_send_msg_to_client(
		zevi->vni, macaddr, ip, flags, seq, ZEBRA_NEIGH_ACTIVE,
				  ...
				  int state, bool force)
{
	...
	return zebra_evpn_macip_send_msg_to_client(
		zevi->vni, macaddr, ip, flags, 0, state, NULL, ZEBRA_MACIP_DEL);
}
...
{
	if (new_bgp_ready)
		zevi_neigh_send_add_to_client(n->zevi, &n->ip, &n->emac, n->mac,
					      n->flags, n->loc_seq);
	else if (old_bgp_ready)
		zevi_neigh_send_del_to_client(n->zevi, &n->ip, &n->emac,
					      n->flags, n->state,
					      true /*force*/);
}
 ...
 */
static void zebra_evpn_sync_neigh_static_chg(zebra_neigh_t *n,
					     bool old_n_static,
...
static bool zebra_evpn_neigh_clear_sync_info(zebra_neigh_t *n)
{
	char macbuf[ETHER_ADDR_STRLEN];
	char ipbuf[INET6_ADDRSTRLEN];
...
/*
 * Delete all neighbor entries for this EVI.
 */
...
static int zebra_evpn_dad_ip_auto_recovery_exp(struct thread *t)
{
	...
	/* Send to BGP */
	if (CHECK_FLAG(nbr->flags, ZEBRA_NEIGH_LOCAL)) {
		zevi_neigh_send_add_to_client(zevi, &nbr->ip, &nbr->emac,
					      nbr->mac, nbr->flags,
					      nbr->loc_seq);
						 ...
						 void *arg)
{
	...
	if (CHECK_FLAG(zn->flags, ZEBRA_NEIGH_LOCAL)
		...
		zevi_neigh_send_add_to_client(wctx->zevi, &zn->ip, &zn->emac,
					      zn->mac, zn->flags, zn->loc_seq);
	}
}

/* Iterator of a specific L2EVI */
...
{
	struct vty *vty;
	json_object *json_evi = NULL, *json_row = NULL;
	zebra_neigh_t *n;
	char buf1[ETHER_ADDR_STRLEN];
	char buf2[INET6_ADDRSTRLEN];
	struct neigh_walk_ctx *wctx = ctxt;
	...
	if (json_evi)
		json_object_object_add(json_evi, buf2, json_row);
}
...
static void zevi_print_neigh_hash_detail(struct hash_bucket *bucket, void *ctxt)
{
	...
	if (json_evi)
		json_object_object_add(json_evi, buf, json_row);
}

/*
 * Print neighbors for all EVI.
 */
void zevi_print_neigh_hash_all_evi(struct hash_bucket *bucket, void **args)
{
	struct vty *vty;
	json_object *json = NULL, *json_evi = NULL;
	zebra_evi_t *zevi;
	uint32_t num_neigh;
	struct neigh_walk_ctx wctx;
	...
	if (json == NULL) {
		...
		json_object_int_add(json_evi, "numArpNd", num_neigh);
...
/*
 * Print neighbors for all EVIs in detail.
 */
				    ...
				    uint32_t seq)
{
	...
	if (!n || !CHECK_FLAG(n->flags, ZEBRA_NEIGH_REMOTE)
	    ...
	    || (memcmp(&n->emac, &mac->macaddr, sizeof(mac->macaddr)) != 0)
	    || !IPV4_ADDR_SAME(&n->r_vtep_ip, &vtep_ip) || seq != n->rem_seq)
		update_neigh = 1;

	if (update_neigh) {
		if (!n) {
			n = zevi_neigh_add(zevi, ipaddr, &mac->macaddr, mac, 0);
			if (!n) {
				zlog_warn(
				  ...
				  struct ipaddr *ip, zebra_mac_t *mac)
{
	...
	n = zevi_neigh_lookup(zevi, ip);
	if (!n) {
		n = zevi_neigh_add(zevi, ip, &mac->macaddr, mac, 0);
		if (!n) {
			flog_err(
				EC_ZEBRA_MAC_ADD_FAILED,
				"Failed to add neighbor %s MAC %s intf %s(%u) -> VNI %u",
