	...
	if (seq < tmp_seq) {
		...
		if (CHECK_FLAG(n->flags, ZEBRA_NEIGH_LOCAL)
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH
				...
				zlog_debug(
					"%s-macip accept evpn %s %s mac %s IP %s lower seq %u f 0x%x",
					sync ? "sync" : "remote", zevpn->name,
					n_type,
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH || IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				"%s-macip ignore evpn %s %s mac %s IP %s as existing has higher seq %u f 0x%x",
				sync ? "sync" : "remote", zevpn->name, n_type,
...
static int zebra_evpn_dad_ip_auto_recovery_exp(struct thread *t)
{
	...
	zevpn = zebra_evpn_lookup(nbr->zevpn->name);
	if (!zevpn)
		return 0;
				     ...
				     bool *is_dup_detect, bool is_local)
{
	...
	if (nbr->dad_count >= zvrf->dad_max_moves) {
		flog_warn(
			...
			"%s: MAC %s IP %s detected as duplicate during %s VTEP %pI4",
			nbr->zevpn->name,
			prefix_mac2str(&nbr->emac, buf, sizeof(buf)),
			ipaddr2str(&nbr->ip, buf1, sizeof(buf1)),
				    ...
				    uint32_t seq)
{
	...
	if (update_neigh) {
		if (!n) {
			...
			if (!n) {
				zlog_warn(
					"Failed to add Neigh %s MAC %s %s Remote VTEP %pI4",
