					       ...
					       bool is_local)
{
	...
	if (mac->dad_count >= zvrf->dad_max_moves) {
		flog_warn(EC_ZEBRA_DUP_MAC_DETECTED,
			  "EVPN %s: MAC %s detected as duplicate during %s VTEP %s",
			  mac->zevpn->name,
			  prefix_mac2str(&mac->macaddr, buf, sizeof(buf)),
				    ...
				    bool force_clear_static, const char *caller)
{
	...
	if (!ifp) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug(
				"%s: dp-install sync-mac evpn %s mac %s es %s 0x%x %sskipped, no access-port",
				caller, zevpn->name,
	...
	if (!br_ifp) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug(
				"%s: dp-install sync-mac evpn %s mac %s es %s 0x%x %sskipped, no br",
				caller, zevpn->name,
				prefix_mac2str(&mac->macaddr, macbuf,
					       sizeof(macbuf)),
				mac->es ? mac->es->esi_str : "-", mac->flags,
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug(
			"dp-install sync-mac evpn %s mac %s es %s 0x%x %s%s",
			zevpn->name,
...
static int zebra_evpn_mac_hold_exp_cb(struct thread *t)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug(
			"sync-mac evpn %s mac %s es %s 0x%x hold expired",
			mac->zevpn->name,
			prefix_mac2str(&mac->macaddr, macbuf, sizeof(macbuf)),
			mac->es ? mac->es->esi_str : "-", mac->flags);
...
static inline void zebra_evpn_mac_start_hold_timer(zebra_mac_t *mac)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug(
			"sync-mac evpn %s mac %s es %s 0x%x hold started",
			mac->zevpn->name,
			prefix_mac2str(&mac->macaddr, macbuf, sizeof(macbuf)),
			mac->es ? mac->es->esi_str : "-", mac->flags);
...
void zebra_evpn_mac_stop_hold_timer(zebra_mac_t *mac)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug(
			"sync-mac evpn %s mac %s es %s 0x%x hold stopped",
			mac->zevpn->name,
			prefix_mac2str(&mac->macaddr, macbuf, sizeof(macbuf)),
			mac->es ? mac->es->esi_str : "-", mac->flags);
...
void zebra_evpn_sync_mac_del(zebra_mac_t *mac)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug(
			"sync-mac del evpn %s mac %s es %s seq %d f 0x%x",
			mac->zevpn->name,
			prefix_mac2str(&mac->macaddr, macbuf, sizeof(macbuf)),
						...
						struct ipaddr *ipaddr)
{
	...
	if (seq < tmp_seq) {
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
				zlog_debug(
					"sync-macip accept evpn %s mac %s%s%s lower seq %u f 0x%x",
					zevpn->name,
					prefix_mac2str(&mac->macaddr, macbuf,
						       sizeof(macbuf)),
					ipa_len ? " IP " : "",
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug(
				"sync-macip ignore evpn %s mac %s%s%s as existing has higher seq %u f 0x%x",
				zevpn->name,
				prefix_mac2str(&mac->macaddr, macbuf,
					       sizeof(macbuf)),
				ipa_len ? " IP " : "",
				...
				struct sync_mac_ip_ctx *ctx)
{
	...
	if (!mac) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC && (old_flags != new_flags))
			zlog_debug(
				"sync-mac evpn %s mac %s old_f 0x%x new_f 0x%x",
				zevpn->name,
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("sync-mac %s evpn %s mac %s es %s seq %d f 0x%x%s%s",
				 ...
				 uint32_t seq, esi_t *esi)
{
	...
	if (update_mac) {
		if (!mac) {
			...
			if (!mac) {
				zlog_warn(
					"Failed to add MAC %s EVPN %s Remote VTEP %s",
			...
			if (seq < tmp_seq) {
				if (IS_ZEBRA_DEBUG_VXLAN)
					zlog_debug(
						"Ignore remote MACIP ADD EVPN %s MAC %s%s%s as existing MAC has higher seq %u flags 0x%x",
						zevpn->name,
						prefix_mac2str(macaddr, buf,
							       sizeof(buf)),
						ipa_len ? " IP " : "",
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
			...
			if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
				zlog_debug(
					"sync-mac->remote evpn %s mac %s es %s seq %d f 0x%x",
					zevpn->name,
					prefix_mac2str(macaddr, buf,
						       sizeof(buf)),
					mac->es ? mac->es->esi_str : "-",
				    ...
				    bool dp_static)
{
	...
	if (!mac) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug(
				"UPD %sMAC %s intf %s(%u) VID %u -> EVPN %s %scurFlags 0x%x",
				sticky ? "sticky " : "",
				prefix_mac2str(macaddr, buf, sizeof(buf)),
				ifp->name, ifp->ifindex, vid, zevpn->name,
				local_inactive ? "local-inactive " : "",
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
			...
			if (CHECK_FLAG(mac->flags, ZEBRA_MAC_STICKY)) {
				flog_warn(
					...
					"MAC %s already learnt as remote sticky MAC behind VTEP %s EVPN %s",
					prefix_mac2str(macaddr, buf,
						       sizeof(buf)),
					inet_ntoa(mac->fwd_info.r_vtep_ip),
					zevpn->name);
	...
	if ((old_local_inactive != local_inactive)
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug(
				"local mac evpn %s mac %s es %s seq %d f 0x%x%s",
				zevpn->name,
				prefix_mac2str(macaddr, buf, sizeof(buf)),
				mac->es ? mac->es->esi_str : "", mac->loc_seq,
				mac->flags,
			     ...
			     struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug(
			"DEL MAC %s intf %s(%u) VID %u -> EVPN %s seq %u flags 0x%x nbr count %u",
	...
	if (zebra_evpn_mac_is_static(mac)) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
			zlog_debug(
				"re-add sync-mac evpn %s mac %s es %s seq %d f 0x%x",
				zevpn->name,
				prefix_mac2str(macaddr, buf, sizeof(buf)),
				mac->es ? mac->es->esi_str : "-", mac->loc_seq,
				mac->flags);
