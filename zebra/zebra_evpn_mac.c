...
uint32_t num_valid_macs(zebra_evi_t *zevi)
{
	...
	zebra_mac_t *mac;
	struct hash *hash = zevi->mac_table;
...
uint32_t num_dup_detected_macs(zebra_evi_t *zevi)
{
	unsigned int i;
	uint32_t num_macs = 0;
	struct hash *hash;
...
static int zebra_evpn_dad_mac_auto_recovery_exp(struct thread *t)
{
	...
	mac = zebra_evpn_mac_lookup(zevi, &mac->macaddr);
	if (!mac)
		return 0;
 ...
 */
static void zebra_evpn_print_mac_hash_detail(struct hash_bucket *bucket,
					     void *ctxt)
{
	struct vty *vty;
					...
					struct zebra_evpn_es *es, uint16_t cmd)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug(
			...
			ipaddr2str(ip, buf2, sizeof(buf2)), seq, id,
			es ? es->esi_str : "-",
...
zebra_mac_t *zebra_evpn_mac_add(zebra_evi_t *zevi, struct ethaddr *macaddr)
{
	...
	mac = hash_get(zevi->mac_table, &tmp_mac, zebra_evpn_mac_alloc);
	assert(mac);
...
static void zevi_mac_del_hash_entry(struct hash_bucket *bucket, void *arg)
{
	...
	if (zebra_evpn_check_mac_del_from_db(wctx, mac)) {
		...
		zebra_evpn_mac_del(wctx->zevi, mac);
	}
 ...
 */
int zevi_mac_send_add_to_client(vni_t vni, struct ethaddr *macaddr,
 ...
 */
int zevi_mac_send_del_to_client(vni_t vni, struct ethaddr *macaddr,
...
static void zebra_evpn_mac_send_add_del_to_client(zebra_mac_t *mac,
						  bool old_bgp_ready,
						  bool new_bgp_ready)
{
	if (new_bgp_ready)
...
static inline bool zebra_evpn_mac_is_bgp_seq_ok(zebra_evi_t *zevi,
...
zebra_mac_t *
zebra_evpn_proc_sync_mac_update(zebra_evi_t *zevi, struct ethaddr *macaddr,
				...
				struct sync_mac_ip_ctx *ctx)
{
	...
	if (!mac) {
		...
		if (!zebra_evpn_mac_is_bgp_seq_ok(zevi, mac, seq, ipa_len,
						  ipaddr)) {
			ctx->ignore_macip = true;
			return NULL;
		}
...
int zebra_evpn_del_local_mac(zebra_evi_t *zevi, struct ethaddr *macaddr,
			     struct interface *ifp)
{
	zebra_mac_t *mac;
	char buf[ETHER_ADDR_STRLEN];
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug(
			...
			ifp->ifindex, mac->fwd_info.local.vid, zevi->vni,
...
void zevi_send_mac_to_client(zebra_evi_t *zevi)
{
	struct mac_walk_ctx wctx;
				 ...
				 uint8_t flags, uint32_t seq, esi_t *esi)
{
	...
	if (update_mac) {
		if (!mac) {
			mac = zebra_evpn_mac_add(zevi, &mac->macaddr);
			if (!mac) {
				zlog_warn(
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
			...
			zebra_evpn_mac_clear_sync_info(mac);
			zevi_mac_send_del_to_client(zevi->vni, &mac->macaddr,
						    mac->flags,
						    false /* force */);
		}
