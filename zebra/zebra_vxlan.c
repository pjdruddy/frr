...
/* definitions */
/* PJDR this must be removed */
void zevi_l3vni_cleanup(struct zebra_vrf *zvrf, zebra_evi_t *zevi);
...
static void zevi_build_hash_table(void)
{
	...
	for (rn = route_top(zns->if_table); rn; rn = route_next(rn)) {
		...
		if (zl3vni) {
			...
			/* VNI hash entry is expected to exist, if the BGP
			 * process is killed */
			zevi = zevi_lookup(vni);
			if (zevi) {
				zlog_debug(
