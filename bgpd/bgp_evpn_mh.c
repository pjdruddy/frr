		...
		struct bgp_node *rn)
{
	...
	if (old_select && old_select == new_select
			...
			&& !bgp_addpath_is_addpath_used(&bgp->tx_addpath,
				afi, safi)) {
		...
		struct prefix_evpn *p)
{
	...
	if (vpn) {
		...
		if (ret != 0) {
			flog_err(EC_BGP_ES_INVALID,
					"%u ERROR: Failed to updated EAD-EVI route VNI %u ESI: %s VTEP %s",
					bgp->vrf_id, vpn->vni, es->esi_str,
					inet_ntoa(es->originator_ip));
		}
		global_rd = &es->prd;
	}
...
static void bgp_evpn_es_evi_remote_info_re_eval(struct bgp_evpn_es_evi *es_evi)
{
	...
	if (listcount(es_evi->es_evi_vtep_list)) {
		...
		if (SET_FLAG(es_evi->flags, BGP_EVPNES_EVI_REMOTE)) {
